#include<stdio.h>					//一个对于长跑运动员有用的程序
...
int main()							
{
	...
	int min,sec,time;				//跑步用时的分钟数和秒数及用秒表示跑步的用时
	...
	scanf("%d",&sec);
	time = min*S_PER_M+sec;			//把时间转换成全部用秒表示
	distm=M_PER_K*distk;			//把公里转换成英里
	rate=distm/time*S_PER_H;		//英里/秒*秒（小时）=英里 每小时<==>英里/time秒*3600秒=英里；
